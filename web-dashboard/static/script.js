...
function initialize3DPlot(id) {
  Plotly.react(
    ...
    [
      {
        type: "scatter3d",
        mode: "lines+markers",
        x: [0], // Titik awal di 0 untuk sumbu X
        y: [0], // Titik awal di 0 untuk sumbu Y
        z: [0], // Titik awal di 0 untuk sumbu Z
        name: "Accel Data",
        marker: {
          size: 4,
          color: id.includes("left") ? "red" : "blue", // Warna untuk tangan kiri dan kanan
        },
        line: {
          width: 4,
    ...
    ],
    {
      ...
      margin: { l: 0, r: 0, b: 0, t: 0 }, // Menjaga margin tetap
      scene: {
        xaxis: { range: [-50, 50] },
        yaxis: { range: [-50, 50] },
        zaxis: { range: [-50, 50] },
...
function updateAccel3D(side, data) {
  ...
  // Simpan data akselerometer ke dalam riwayat
  if (side === "left") {
    hist.x.push(-data.accel[0]); // Membalikkan X untuk tangan kiri
  } else {
    hist.x.push(data.accel[0]); // Biarkan X tangan kanan tetap
  }

  hist.y.push(data.accel[1]);
  hist.z.push(data.accel[2]);

  // Batasi hanya 10 titik terakhir agar tidak menumpuk
  if (hist.x.length > 10) {
    hist.x.shift();
    hist.y.shift();
    hist.z.shift();
  ...
  // Menggunakan Plotly.react() untuk memperbarui plot dengan data baru
  Plotly.react(
    plotId,
    [
      {
